...
import pf.File
import pf.Arg
import "data/DerivedCoreProperties.txt" as data : Str
import Helpers
...
propertyMapFromFile = \file, parsePropPart ->
    ...
    |> List.keepIf \{ cp: _, prop: (label, _value) } -> label == "Case_Ignorable" || label == "Cased"
...
ignorablesTrie =
    propertyMap
    |> collectLabelled "Case_Ignorable"
    |> Helpers.toTrie
    |> Helpers.trieToSwitch 4 "Bool.false"

casedTrie =
    propertyMap
    |> collectLabelled "Cased"
    |> Helpers.toTrie
    |> Helpers.trieToSwitch 4 "Bool.false"
...
main =
    ...
    when Arg.list! {} |> List.get 1 is
        Err _ -> Task.err (InvalidArguments "USAGE: roc run InternalDerivedPropsGen.roc -- path/to/package/")
        Ok arg ->
            File.writeUtf8 template "$(Helpers.removeTrailingSlash arg)/InternalDerivedProps.roc"
